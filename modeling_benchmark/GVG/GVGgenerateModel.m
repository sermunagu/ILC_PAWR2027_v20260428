...
for igen = 1:GVGconfig.ngenerations
    tic; fprintf('--------------------- Generation %d ---------------------\n',igen)
    
    %%% Change to distinguish between the first generation, in which
    %%%  initialization of the models is done, and the rest
    if igen > 1
        rManager.initgen = 0;
    end
    ...
    if(GVGconfig.validate && rem(igen,GVGconfig.validatengen)==0)
        ...
        normU = vecnorm(U);
        Un = U*diag(normU.^(-1));
        %yvalmod = (U*(diag(normU.^-1)*((pinv(Un)*yU))));
        yvalmod = U*rManagerv{igen}.h(rManagerv{igen}.s,rManagerv{igen}.nopt);
        %nmseval =20*log10(norm(yvalmod-yU,2)/norm(yU,2));
    ...
    if igen~=GVGconfig.ngenerations
        rManager.crossover();
        rManager.mutation();
        rManager.removerepeated();
        rManager.printModel();
