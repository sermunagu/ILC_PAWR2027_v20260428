classdef Regressor < handle
    ...
    methods
        ...
        function self = buildRegressor(self, x, n, Qpmax, Qnmax)
            if isempty(self.reg)
                self.reg = ones(size(n(1+Qpmax:end-Qnmax)));
                for i=1:length(self.X)
                    self.reg = self.reg.*x(n(1+Qpmax-self.X(i):end-Qnmax-self.X(i)));
                end
                for i=1:length(self.Xconj)
                    self.reg = self.reg.*conj(x(n(1+Qpmax-self.Xconj(i):end-Qnmax-self.Xconj(i))));
                end
                for i=1:length(self.Xenv)
                    self.reg = self.reg.*abs(x(n(1+Qpmax-self.Xenv(i):end-Qnmax-self.Xenv(i))));
                end
            end
        end
